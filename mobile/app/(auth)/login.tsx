...
export default function LoginScreen() {
  ...
  async function handleLogin() {
    if (!email || !password) { Alert.alert(i18n('fillAllFields')); return }
    setLoading(true)
  ...
  async function handleGoogle() {
    ...
    if (error || !data.url) {
      ...
      Alert.alert('Error', i18n('googleError'))
      return
    }
  ...
  return (
    <KeyboardAvoidingView style={[styles.flex, { backgroundColor: t.bg }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <Text style={[styles.title, { color: t.text }]}>{i18n('welcome')}</Text>
        <Text style={[styles.subtitle, { color: t.text3 }]}>{i18n('loginSubtitle')}</Text>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.field}>
            <Text style={[styles.label, { color: t.text2 }]}>{i18n('email')}</Text>
            <TextInput
              ...
              placeholderTextColor={t.text4} keyboardType="email-address" autoCapitalize="none" autoComplete="email"
            />
          </View>

          <View style={styles.field}>
            <Text style={[styles.label, { color: t.text2 }]}>{i18n('password')}</Text>
            <TextInput
              style={[styles.input, { backgroundColor: t.card, borderColor: t.border2, color: t.text }]}
              value={password} onChangeText={setPassword} placeholder="••••••••"
              placeholderTextColor={t.text4} secureTextEntry autoComplete="password"
            />
          </View>

          <TouchableOpacity style={styles.btn} onPress={handleLogin} disabled={loading} activeOpacity={0.85}>
            {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>{i18n('login')}</Text>}
          </TouchableOpacity>

          <TouchableOpacity onPress={() => router.push('/(auth)/register')} style={styles.link}>
            <Text style={[styles.linkText, { color: t.text3 }]}>
              {i18n('noAccount')} <Text style={styles.linkBold}>{i18n('register')}</Text>
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  form: { gap: 14 },
  googleBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, borderWidth: 1.5, borderRadius: 16, paddingVertical: 14 },
